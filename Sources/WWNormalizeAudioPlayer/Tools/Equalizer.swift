...
extension WWNormalizeAudioPlayer {
    ...
    public class Equalizer {
        ...
        let audioNode: AVAudioUnitEQ    // 底層的 AVAudioUnitEQ 實例
        ...
        init(frequencies: [Float] = [32, 63, 125, 250, 500, 1000, 2000, 4000, 8000, 16000]) {
            ...
            self.audioNode = AVAudioUnitEQ(numberOfBands: frequencies.count)
...
public extension WWNormalizeAudioPlayer.Equalizer {
    ...
    var globalGain: Float {
        get { audioNode.globalGain }
        set { audioNode.globalGain = newValue }
    }
}
...
public extension WWNormalizeAudioPlayer.Equalizer {
    ...
    func setEnabled(_ enabled: Bool) { audioNode.bypass = !enabled }
    ...
    func bandGain(index: Int, gain: Float) throws {
        guard audioNode.bands.indices.contains(index) else { throw WWNormalizeAudioPlayer.CustomError.equalizerBandIndexOutOfRange(index: index, count: audioNode.bands.count) }
        audioNode.bands[index].gain = gain
    }
    ...
    func bandFrequency(index: Int, frequency: Float) throws {
        guard audioNode.bands.indices.contains(index) else { throw WWNormalizeAudioPlayer.CustomError.equalizerBandIndexOutOfRange(index: index, count: audioNode.bands.count) }
        audioNode.bands[index].frequency = frequency
    }
    ...
    func bandBandwidth(index: Int, bandwidth: Float) throws {
        guard audioNode.bands.indices.contains(index) else { throw WWNormalizeAudioPlayer.CustomError.equalizerBandIndexOutOfRange(index: index, count: audioNode.bands.count) }
        audioNode.bands[index].bandwidth = bandwidth
    }
    ...
    func bandType(index: Int, type: WWNormalizeAudioPlayer.EqualizerBandType) throws {
        guard audioNode.bands.indices.contains(index) else { throw WWNormalizeAudioPlayer.CustomError.equalizerBandIndexOutOfRange(index: index, count: audioNode.bands.count) }
        audioNode.bands[index].filterType = map(type)
    }
    ...
    func bandBypass(index: Int, bypass: Bool) throws {
        guard audioNode.bands.indices.contains(index) else { throw WWNormalizeAudioPlayer.CustomError.equalizerBandIndexOutOfRange(index: index, count: audioNode.bands.count) }
        audioNode.bands[index].bypass = bypass
    }
    ...
    func band(index: Int, type: WWNormalizeAudioPlayer.EqualizerBandType? = nil, frequency: Float? = nil, bandwidth: Float? = nil, gain: Float? = nil, bypass: Bool? = nil) throws {
        ...
        let band = audioNode.bands[index]
    ...
    func preset(_ preset: WWNormalizeAudioPlayer.EqualizerPreset) {
        ...
        switch preset {
        case .flat: apply(gains: Array(repeating: 0, count: frequencies.count)); audioNode.globalGain = 0
...
private extension WWNormalizeAudioPlayer.Equalizer {
    ...
    func configureDefaultBands() {
        ...
        for (index, band) in audioNode.bands.enumerated() {
            band.bypass = false
